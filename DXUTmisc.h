...
public:
	...
	HRESULT LastIndexOf( const TYPE& value, int nIndex, int nNumElements );
...
protected:
    ...
    long SetSizeInternal( int nNewMaxSize );  // This version doesn't call ctor or dtor.
};
...
//--------------------------------------------------------------------------------------
template<typename TYPE> HRESULT CGrowableArray <TYPE>::LastIndexOf( const TYPE& value, int iEnd, int nNumElements )
{
    // Validate arguments
    if( iEnd < 0 ||
        iEnd >= m_nSize ||
